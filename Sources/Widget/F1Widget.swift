...
struct Provider: TimelineProvider {
    func placeholder(in context: Context) -> SimpleEntry {
        SimpleEntry(date: Date(), raceName: "MONACO GRAND PRIX", raceDate: "2024-05-26", results: F1DataService.shared.getMockResults())
    }
    ...
    func getTimeline(in context: Context, completion: @escaping (Timeline<SimpleEntry>) -> ()) {
        Task {
            let (results, raceName, raceDate) = (try? await F1DataService.shared.fetchLatestResults()) ?? (F1DataService.shared.getMockResults(), "LATEST RACE", "")
            let entry = SimpleEntry(date: Date(), raceName: raceName, raceDate: raceDate, results: results)
...
struct F1Widget_Previews: PreviewProvider {
    static var previews: some View {
        ...
        let dummyEntry = SimpleEntry(date: Date(), raceName: "MONACO GRAND PRIX", raceDate: "2024-05-26", results: dummyResults)
